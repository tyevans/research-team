 ...
 *
 * **MATERIAL's floor moves with its tab strip, and has been re-measured six
 ...
 * (537 against 581.6), once when Classes made it a ninth (582 against
 * 645.875), and once when Curriculum made it an eleventh (646 against
 * 780.703125), all in Chromium.
 *
 * **The row now has a scroller, and this paragraph used to say it did not.**
 * `.tabs` gained `overflow-x: auto` in the same change as the eleventh tab,
 * because `project-stacked.browser.test.tsx` had predicted the clip and left
 * the decision open. That changes what a lagging floor costs but not whether
 * it costs: a strip that overflows is scrollable rather than truncated, so the
 * newest tab is reachable instead of "present, painted, unclickable" -- which
 * is what the old unmeasured `280/320/280` did to the Graph tab on a page that
 * had only ever been looked at at 1440. It is still worth keeping the floor
 * ahead of the strip, because a tab a reader has to scroll to is a tab most
 * readers never find.
 *
 * **784 does not bind anywhere in the wide band, and is written down anyway.**
 * MATERIAL is `1fr` beside a quarter-width sidebar, so it takes 837 at 1181 and
 * more above -- fifty pixels of clearance now, where 646 had three hundred.
 * That is the honest state of it: the eleventh tab spent most of the slack, and
 * a twelfth does not fit the wide band at all without either shorter labels or
 * a wider breakpoint. The number is the guard, not the geometry: it is what
 * fails, at the width where a reader would lose a tab, if a twelfth arrives and
 * this line does not move with it. The measurement
...
export const PROJECT_TRACKS: readonly Track[] = [
  ...
  // 646 -> 784: the eleventh tab, Curriculum. **Measured at 780.703125px** by
  // `project-tracks.browser.test.tsx`, which is the test that exists to fail
  // when a floor is invalidated by a tab -- it caught the seventh the same way
  // (`expected 422 to be greater than or equal to 536.3125`). Rounded up to
  // 784 rather than set to the measurement, so the next label change of a few
  // pixels does not immediately re-trip it.
  //
  // Still fits: 344 + 784 = 1128 against `BP_WIDE` 1181.
