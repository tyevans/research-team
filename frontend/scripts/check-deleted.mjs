...
const RULES = [
  ...
  {
    phase: '5',
    what: '`AutonomyPanel` was dressed from `course.css`',
    why: "The `AutonomyAllowAll` trap again, on the other surface over the same policy, and this time the move is what made porting mandatory. The panel left the project page's queue header for a dialog behind the chrome's lock, so it is reached on every route while `course.css` is on the die-with-its-screen list -- deleting that file would have taken the row rule, the fieldset reset, the scope warning's accent border and eleven others off a panel still on screen, with nothing failing: jsdom applies no stylesheet and a class that resolves to nothing raises no error. The fifteen rules are utilities on `AutonomyPanel.tsx`. Forbidden under `styles` rather than merely deleted because an unlayered `.autonomy-row` reclaiming the name would beat the utilities outright, which is the split this deletion closed.",
    where: 'styles',
    forbid: [
      /^\.autonomy-head\b/m,
      /^\.autonomy-title\b/m,
  ...
  },
  {
    ...
    why: "The other half of the rule above. `.autonomy-warn` and `.autonomy-error` stay in `course.css` for `AutonomyPanel`, which is a course-page surface and dies with the file -- but `AutonomyAllowAll` renders in the shell's decision bar on every route, so writing those classes there was a control borrowing dressing from a screen it outlives. It carries the same declarations as utilities now. Scoped to the one file by `only`, because the directory also held `AutonomyPanel.tsx`, which went on using them. **Both halves of that split are gone**: `presentation/course/` no longer exists, so this rule matches nothing, and the panel's move into the chrome took `.autonomy-warn` and `.autonomy-error` out of `course.css` altogether -- they are in the phase-5 `styles` rule above now, forbidden for both surfaces rather than for one. Kept rather than deleted because removing a rule is a decision somebody makes in a diff, and the sentence it now carries is the record of why it is inert.",
    where: 'presentation/course',
