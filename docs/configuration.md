...
| `AGENT_CATALOG_SWEEP_CONCURRENCY` | `4` | how many candidates a catalog blurb or art sweep works on at once. Deliberately well below `AGENT_EXTRACTION_CONCURRENCY`: that ceiling is per *document* over 2000-character chunks, where a sweep's unit is a whole blurb, outline or SVG with a slot held for the length of the generation. Chosen conservatively rather than from a curve — the endpoint was too contended on the day to measure a knee, and the config docstring records both the numbers and why they cannot be read as a comparison. Set to `1` for the sequential behaviour this replaced; lower it for a hosted endpoint with a per-minute quota |
