...
### B101. Forking a research-round session gives a person a session still labelled `RESEARCH_ROUND` — CLOSED 2026-09-20

`SessionService.fork` now supports `purpose: SessionPurpose | None = None`.
When provided (and through `POST /api/sessions/{id}/forks` via `NewFork.purpose`),
the forked stream has its `SessionStarted.purpose` retargeted and records the
lineage purpose in `SessionForkedFrom(purpose=...)`. Callers omitting `purpose`
faithfully retain the source purpose, while interactive console users and tools
can retarget automated research or seeding sessions directly to `CHAT`. Covered
in `tests/application/test_session_service.py` and `tests/interfaces/test_web_sessions.py`.
