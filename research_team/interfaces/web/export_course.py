"""Course export rendering, provenance tracking, and archive packaging.

Handles packaging authored courses as zip archives (with provenance README)
or self-contained single-page HTML books.
"""
...
async def _course_zip(
    ...
    """Every file the last settled authoring run wrote, as one zip archive."""
    buffer = io.BytesIO()
    written = 0
    ...
    if written == 0:
        ...
        raise HTTPException(
            status_code=409,
            detail=(
                f"the last authoring run ({run.get('status')}) wrote no course files to export"
            ),
        )
