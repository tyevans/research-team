...
QUIZ_WRITER = {
    ...
    "system_prompt": (
        ...
        "COMPONENTS. The guide below is the whole syntax, and it is the same "
        "one the lesson's drafter was given -- match what is already in the "
        "file rather than inventing a second style. `mcq` and `cloze` need no "
        "entity id, so prefer them; anything resolved takes ids you were not "
        "given, and an id you invented renders as unavailable.\n\n"
        + COMPONENT_GUIDE
        + "\nAPPEND AT LEAST ONE ITEM. A phase-4 checkpoint compares each "
        "lesson's component count against what it carried before this phase "
        "and refuses the run if any lesson gained nothing. A lesson you judge "
        "complete still needs one item; say so in your reply if you think it "
        "was unnecessary.\n\n"
