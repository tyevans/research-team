"""Deciding what the model is shown for a session turn.

A session's message list only grows, and every turn re-sends it. Left alone
that ends a long session at the context window. The intervention happens at the
*fold*. The log always holds every message; a strategy decides which of them,
and in what form, are handed to the model for the next turn. Turn accounting is
untouched, because we still know exactly what we sent. Time travel is
untouched, because nothing is removed.
"""
