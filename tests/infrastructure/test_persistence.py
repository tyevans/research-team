...
async def test_session_survives_a_closed_store(fake_model, db_path, build_service, repository):
    first = build_service(model=fake_model, db_path=db_path)
    session_id = await first.create_session()
    await first.run_turn(session_id, "remember this")
    await first.close()

    build_service(model=fake_model, db_path=db_path)
...
async def test_reopening_keeps_the_stored_system_prompt(fake_model, db_path, build_service):
    first = build_service(model=fake_model, db_path=db_path, system_prompt="ORIGINAL")
    session_id = await first.create_session()
    await first.close()

    reopened = build_service(model=fake_model, db_path=db_path, system_prompt="DIFFERENT")
